...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        # Note: Omitted alpha here because alpha set to 1
        return inputs * (inputs > 0) + ( inputs < 0) * self.alpha * np.expm1(inputs)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return outputs*(outputs>0) + (outputs<0)*((np.exp(outputs)) + self.alpha) * grads_wrt_outputs 
...
class GELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        return 0.5 * inputs (1 + np.tanh(np.sqrt(2/pi) * (inputs + 0.044715 * x**3)))
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return (0.5 * (np.tanh*0.0356774*outputs**3 + 0.797885 * outputs) + 0.0535161 * outputs**3 + 0.398942*outputs) * sech(0.0356774*outputs**3 + 0.797885 * outputs)**2 + 0.5) * grads_wrt_outputs
...
class IRSLULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        return inputs*(inputs>0) + (inputs<0)* (inputs * (1/np.sqrt(1+alpha * inputs**2)))
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return (outputs>0) + (outputs<0)(1/(np.sqrt(1+alpha * x**2))) * grads_wrt_outputs
