...
class GlorotNormalInit(object):
    ...
    def __call__(self, shape):
        if type(shape) != int:        
            # the shape must have two dimensions
            std = self.gain * (2. / (shape[0] + shape[1]))**0.5
            return self.rng.normal(loc=0., scale=std, size=shape)
        else:
            std = 1 # we set a std. deviation of 1 for the biases initialization step 
            return self.rng.normal(loc=0., scale=std, size=shape)
    
    
class LeCunNormalInit(object):
    """
        A modified version of the true LeCun normal. The true version draws samples from a truncated normal distribution, whereas our implementation uses rng.normal without truncation.
    """

    def __call__(self, shape):
        stddev = 1 / .87962566103423978
        x = rng.normal(loc=mu, scale=stddev, size=shape)
        return x
