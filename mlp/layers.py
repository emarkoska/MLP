...
class AffineLayer(LayerWithParameters):
    ...
    def __init__(self, input_dim, output_dim,
        ...
        self.weights = weights_initialiser((self.output_dim, self.input_dim)) # we use the object to make a _call__
        self.biases = biases_initialiser(self.output_dim) # we do the same here
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        return inputs * (inputs > 0) + (inputs <= 0) * np.expm1(inputs) * self.alpha
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return 1 * (outputs > 0) + (outputs <= 0) * (self.alpha * np.exp(outputs)) * grads_wrt_outputs 
...
class GELULayer(Layer):
    """Layer implementing a Gaussian Error Linear Unit."""
    ...
    def fprop(self, inputs):
        """Forward propagates activations through the layer transformation."""
        pie = 3.1415926
        return 0.5 * inputs * (1 + np.tanh(np.sqrt(2/pie) * (inputs + 0.044715 * inputs**3)))
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        x = outputs
        t = np.tanh(0.0356774 * x ** 3 + 0.797885 * x)
        s = self.sech(0.0356774 * x ** 3 + 0.797885 * x)

        return (0.5 * t + (0.0535161 * x ** 3 + 0.398942 * x) * s ** 2 + 0.5) * grads_wrt_outputs
...
class ISRLULayer(Layer):
    """Layer implementing a Inverse Square Root Linear Unit."""
    ...
    def fprop(self, inputs):
        ...
        return inputs*(inputs>=0) + (inputs<0)* (inputs * (1/np.sqrt(1+self.alpha * inputs**2)))
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return (outputs>=0) + (outputs<0)*(1/(np.sqrt(1+self.alpha * outputs **2)))**3 * grads_wrt_outputs
